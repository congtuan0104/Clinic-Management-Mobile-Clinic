...
export default function CalendarScreen({ navigation }: CalendarNavigatorProps)  {
  ...
  const [timelineEvents, setTimelineEvents] = useState<TimelineEventsState[] | undefined>();
  ...
  const markedDate = useMemo(() => (appointments.map((item) => (
    {
    date: item.startTime, 
    dots: [
      {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        console.log('timeline events after getTimelineEvents: ', timelineEvents);
  ...
  const getTimelineEvents = async () => {
    ...
    const newArray = todoList?.map((item: IAppointment) => {      
      ...
      return ({
      time: startTimeString.slice(startTimeString.indexOf('T') + 1, startTimeString.indexOf('T') + 6),
  ...
  return (
    <Fragment>
      ...
      >
        ...
        <TouchableOpacity
          onPress={() =>
            navigation.navigate("CreateTaskNavigator")
          }
