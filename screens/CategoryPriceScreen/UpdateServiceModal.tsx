import {
  ...
  import { ICategory, IClinicService, IPostClinicServiceParams } from "../../types";
  ...
  export default function UpdateServiceModal({
    ...
    const getCategoryList = async () => {
        try {
          if (clinic?.id)
          {
            const response = await categoryApi.getCategories(clinic!.id, { type: CATEGORY_TYPE.SERVICE });
    ...
    return (
      <Modal isOpen={isOpen} onClose={onClose} >
        ...
        <Modal.Content width="90%">
          <Modal.CloseButton />
          <Modal.Header><Text fontWeight={"bold"}>Cập nhật thông tin dịch vụ</Text></Modal.Header>
          <Modal.Body>
            <Box>
              <ScrollView minWidth="100%" maxWidth="100%">
                ...
                <FormControl isRequired isInvalid={errors.name ? true : false}>
                  ...
                  {/* Trạng thái */}
                  <FormControl isRequired isInvalid={errors.status ? true : false}>
                    <FormControl.Label
                      _text={{
                        bold: true,
                      }}
                    ...
                    <Controller
                      ...
                      render={({ field: { onChange, onBlur, value } }) => (
                        <SelectDropdown
                            ...
                            defaultButtonText={service.isDisabled? "Không hoạt động" : "Đang hoạt động"}
                            buttonTextAfterSelection={(selectedItem, index) => {
                                return selectedItem;
                            }}
                            rowTextForSelection={(item, index) => {
          ...
          <Modal.Footer>
            <Button.Group space={2}>
              ...
              <Button onPress={handleSubmit(handleSubmitForm, onInvalid)}>Lưu thay đổi</Button>
            </Button.Group>
          </Modal.Footer>
        </Modal.Content>
      </Modal>
